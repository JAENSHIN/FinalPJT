...
function initializeMap() {
    ...
    function updateAIRecommendations(recommendation) {
        ...
        const recommendations = recommendation.split(/\d+\.\s+/).filter(rec => rec.trim() !== '');

        recommendations.forEach((rec, index) => {
            const [item, reason] = rec.split('이유:').map(part => part.trim());
        ...
        if (data && Array.isArray(data.body.items)) {
            ...
            data.body.items.forEach(business => {
                ...
                kakao.maps.event.addListener(marker, 'click', function () {
                    displayInfoWindow(marker, groupedBusinesses[key]);
                });
            });
        }
    }
